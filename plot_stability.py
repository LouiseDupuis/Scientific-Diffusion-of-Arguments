...
def plot_community_data(stability_data):
    fig = plt.figure()
...
adress = "MABS_data\mv_data_1_agents_stability.csv"

def get_indiv_data(adress):
       indiv_data = pd.read_csv(adress)
       print(indiv_data)

       return indiv_data

data = get_indiv_data(adress)

def plot_individual_data(data):
        ...
        cmap = plt.cm.get_cmap('tab10')
        #colors = [cmap[0], cmap[0.5], cmap[1]]
        averages_dict=dict([(0.001, []), (0.2, []), (0.6, []) ])
        for column in data:
                T, sigma = column.split(" ")
                
                if T != "0" and T != 'Unnamed:':
                    sigma = float(sigma)
                    T = float(T)
                
                    points = [int(p) for p in data[column].values]
                    print(points, sigma, T)
                    #color = colors[N]

                    xs = [sigma for i in range(len(points))]
                    ys = [T for i in range(len(points))]
                    za = np.average(points)
                    zs = points
                    ax.scatter(xs, ys, zs, marker=m, color=cmap(sigma))
                    ax.scatter(xs, ys, za, marker='^', color='black', linewidth=5)
                    averages_dict[sigma] += [(sigma, T,za)]
        
        for sig in averages_dict.keys():
            ax.plot([sigma for sigma,T,av in averages_dict[sig]], [T for sigma,T,av in averages_dict[sig]], [av for igma, T,av in averages_dict[sig]], color="black")

        ft_size = 15
        ax.set_xlabel('Experimental accuracy $\sigma$')
        ax.set_ylabel('$T_i$')
        ax.set_zlabel('First step of stability $t_s$')
        plt.xticks(fontsize = ft_size)
        plt.yticks(fontsize = ft_size)
